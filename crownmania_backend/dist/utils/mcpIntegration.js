...
class MCPIntegration extends EventEmitter {
  ...
  handleWebSocketMessage(ws, data) {
    switch (data.type) {
      case 'subscribe':
        ...
        break;
      case 'getErrors':
        this.fetchLocalErrors().then(errors => {
  ...
  async fetchLocalErrors() {
    try {
      ...
      const newEntries = lines.slice(this.lastReadPosition).map(entry => this.parseLogEntry(entry)).filter(entry => entry !== null);
  ...
  async lintCode(code) {
    try {
      const response = await axios.post(`${this.baseUrl}/lint`, {
        code
      });
