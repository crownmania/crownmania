...
export const userSchema = Joi.object({
  ...
  walletAddress: Joi.string().pattern(/^0x[a-fA-F0-9]{40}$/).message('Invalid Ethereum wallet address format').required(),
...
export const serialNumberSchema = Joi.object({
  serialNumber: Joi.string().pattern(/^[A-Z0-9]{6,20}$/).message('Serial number must be 6-20 characters long and contain only uppercase letters and numbers').required(),
...
export const walletSchema = Joi.object({
  walletAddress: Joi.string().pattern(/^0x[a-fA-F0-9]{40}$/).message('Invalid Ethereum wallet address format').required(),
  signature: Joi.string().pattern(/^0x[a-fA-F0-9]{130}$/).message('Invalid signature format').required(),
...
export const mintingSchema = Joi.object({
  serialNumber: Joi.string().pattern(/^[A-Z0-9]{6,20}$/).required(),
  recipientAddress: Joi.string().pattern(/^0x[a-fA-F0-9]{40}$/).message('Invalid recipient wallet address').required(),
  metadata: Joi.object({
...
export const shippingSchema = Joi.object({
  trackingNumber: Joi.string().pattern(/^[A-Z0-9]{6,30}$/).message('Invalid tracking number format').required(),
