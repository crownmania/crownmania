...
class Collectible {
  ...
  static async findBySerialNumber(serialNumber) {
    try {
      const snapshot = await db.collection('collectibles').where('serialNumber', '==', serialNumber).limit(1).get();
  ...
  static async findByOwner(ownerId) {
    try {
      const snapshot = await db.collection('collectibles').where('ownerId', '==', ownerId).get();
      return snapshot.docs.map(doc => new Collectible({
        id: doc.id,
        ...doc.data()
      }));
    } catch (error) {
