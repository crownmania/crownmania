...
export const verifySerialNumber = async (req, res) => {
  try {
    ...
    // Verify reCAPTCHA
    const recaptchaResponse = await axios.post(`https://www.google.com/recaptcha/api/siteverify?secret=${process.env.RECAPTCHA_SECRET_KEY}&response=${recaptchaToken}`);
...
export const claimCollectible = async (req, res) => {
  try {
    ...
    if (collectible.status !== 'unclaimed') {
      return res.status(400).json({
        error: 'Collectible already claimed'
      });
    }
